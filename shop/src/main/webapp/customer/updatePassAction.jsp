...
<%
	...
	if(customerService.modifyCustomerPass(customer)) {
		...
		response.sendRedirect(request.getContextPath()+"/customer/customerList.jsp?updatePass");
	} else {
		System.out.println("고객 패스워드변경 실패");
		response.sendRedirect(request.getContextPath()+"/customer/customerList.jsp?updateFail");
	}
